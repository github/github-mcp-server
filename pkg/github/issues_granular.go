	...
	st := NewTool(
									...
									Properties: map[string]*jsonschema.Schema{
										...
										"rationale": {
											...
											Description: "One concise sentence explaining what specifically about the issue led you to choose this assignee. " +
												"State the concrete signal (e.g. 'Author of the component mentioned in the bug report').",
										...
										"is_suggestion": {
											Type: "boolean",
											Description: "If true, this assignee is sent to the API as a suggestion (suggest:true) rather than an applied assignment. " +
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			if useObjectForm {
				...
				body = &github.IssueRequest{Assignees: &names}
			}
