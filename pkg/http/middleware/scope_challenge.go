...
func WithScopeChallenge(oauthCfg *oauth.Config, scopeFetcher scopes.FetcherInterface) func(http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			...
			challengeScopes := toolScopeInfo.ChallengeScopes(activeScopes...)
			...
			recommendedScopes := make([]string, 0, len(activeScopes)+len(challengeScopes))
			recommendedScopes = append(recommendedScopes, activeScopes...)
			recommendedScopes = append(recommendedScopes, challengeScopes...)
